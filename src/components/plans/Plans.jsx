import React from "react";

export default function Plans() {
  return (
    <>
      <h2 className="p-relative">Plans</h2>
      <div className="plans-page d-grid m-20 gap-20">
        <div className="plan border-ccc blue rad-6 bg-white p-20">
          <div className="top txt-c p-20 bg-blue">
            <h2 className="m-0 c-white">Free</h2>
            <div className="price c-white">
              <span>$</span>0.00
            </div>
          </div>
          <ul>
            <li>
              <i className="fa-solid fa-check fa-fw yes c-green mr-5"></i>
              <span>Access All Text Lesson</span>
              <i className="fa-solid fa-circle-info fa-fw help"></i>
            </li>
            <li>
              <i className="fa-solid fa-check fa-fw yes c-green mr-5"></i>
              <span>Access All Video Lesson</span>
              <i className="fa-solid fa-circle-info fa-fw help"></i>
            </li>
            <li>
              <i className="fa-solid fa-check fa-fw yes c-green mr-5"></i>
              <span>Appear On headboard</span>
              <i className="fa-solid fa-circle-info fa-fw help"></i>
            </li>
            <li>
              <i className="fa-solid fa-xmark fa-fw c-red mr-5"></i>
              <span>Access All Assignment</span>
              <i className="fa-solid fa-circle-info fa-fw help"></i>
            </li>
            <li>
              <i className="fa-solid fa-xmark fa-fw c-red mr-5"></i>
              <span>Get Daily Prices</span>
              <i className="fa-solid fa-circle-info fa-fw help"></i>
            </li>
            <li>
              <i className="fa-solid fa-xmark fa-fw c-red mr-5"></i>
              <span>Earn Certification</span>
              <i className="fa-solid fa-circle-info fa-fw help"></i>
            </li>
            <li>
              <i className="fa-solid fa-xmark fa-fw c-red mr-5"></i>
              <span>1 GB Space For Hosting Files</span>
              <i className="fa-solid fa-circle-info fa-fw help"></i>
            </li>
            <li>
              <i className="fa-solid fa-xmark fa-fw c-red mr-5"></i>
              <span>Access Badge System</span>
              <i className="fa-solid fa-circle-info fa-fw help"></i>
            </li>
          </ul>
          <a className="s-btn-shape bg-blue c-white d-block w-fit txt-c mt-10 blue" href="j">Join</a>
        </div>
        <div className="plan green border-ccc rad-6 bg-white p-20">
          <div className="top txt-c p-20 bg-green">
            <h2 className="m-0 c-white">Pro</h2>
            <div className="price c-white">
              <span>$</span>100.00
            </div>
          </div>
          <ul>
            <li>
              <i className="fa-solid fa-check fa-fw yes c-green mr-5"></i>
              <span>Access All Text Lesson</span>
              <i className="fa-solid fa-circle-info fa-fw help"></i>
            </li>
            <li>
              <i className="fa-solid fa-check fa-fw yes c-green mr-5"></i>
              <span>Access All Video Lesson</span>
              <i className="fa-solid fa-circle-info fa-fw help"></i>
            </li>
            <li>
              <i className="fa-solid fa-check fa-fw yes c-green mr-5"></i>
              <span>Appear On headboard</span>
              <i className="fa-solid fa-circle-info fa-fw help"></i>
            </li>
            <li>
              <i className="fa-solid fa-check fa-fw yes c-green mr-5"></i>
              <span>Access All Assignment</span>
              <i className="fa-solid fa-circle-info fa-fw help"></i>
            </li>
            <li>
              <i className="fa-solid fa-check fa-fw yes c-green mr-5"></i>
              <span>Get Daily Prices</span>
              <i className="fa-solid fa-circle-info fa-fw help"></i>
            </li>
            <li>
              <i className="fa-solid fa-check fa-fw yes c-green mr-5"></i>
              <span>Earn Certification</span>
              <i className="fa-solid fa-circle-info fa-fw help"></i>
            </li>
            <li>
              <i className="fa-solid fa-xmark fa-fw c-red mr-5"></i>
              <span>1 GB Space For Hosting Files</span>
              <i className="fa-solid fa-circle-info fa-fw help"></i>
            </li>
            <li>
              <i className="fa-solid fa-xmark fa-fw c-red mr-5"></i>
              <span>Access Badge System</span>
              <i className="fa-solid fa-circle-info fa-fw help"></i>
            </li>
          </ul>
          <a className="s-btn-shape bg-green c-white d-block w-fit txt-c mt-10 green" href="f">Join</a>
        </div>
        <div className="plan orange border-ccc rad-6 bg-white p-20">
          <div className="top txt-c p-20 bg-orange">
            <h2 className="m-0 c-white">Ult</h2>
            <div className="price c-white">
              <span>$</span>500.00
            </div>
          </div>
          <ul>
            <li>
              <i className="fa-solid fa-check fa-fw yes c-green mr-5"></i>
              <span>Access All Text Lesson</span>
              <i className="fa-solid fa-circle-info fa-fw help"></i>
            </li>
            <li>
              <i className="fa-solid fa-check fa-fw yes c-green mr-5"></i>
              <span>Access All Video Lesson</span>
              <i className="fa-solid fa-circle-info fa-fw help"></i>
            </li>
            <li>
              <i className="fa-solid fa-check fa-fw yes c-green mr-5"></i>
              <span>Appear On headboard</span>
              <i className="fa-solid fa-circle-info fa-fw help"></i>
            </li>
            <li>
              <i className="fa-solid fa-check fa-fw yes c-green mr-5"></i>
              <span>Access All Assignment</span>
              <i className="fa-solid fa-circle-info fa-fw help"></i>
            </li>
            <li>
              <i className="fa-solid fa-check fa-fw yes c-green mr-5"></i>
              <span>Get Daily Prices</span>
              <i className="fa-solid fa-circle-info fa-fw help"></i>
            </li>
            <li>
              <i className="fa-solid fa-check fa-fw yes c-green mr-5"></i>
              <span>Earn Certification</span>
              <i className="fa-solid fa-circle-info fa-fw help"></i>
            </li>
            <li>
              <i className="fa-solid fa-check fa-fw yes c-green mr-5"></i>
              <span>1 GB Space For Hosting Files</span>
              <i className="fa-solid fa-circle-info fa-fw help"></i>
            </li>
            <li>
              <i className="fa-solid fa-check fa-fw yes c-green mr-5"></i>
              <span>Access Badge System</span>
              <i className="fa-solid fa-circle-info fa-fw help"></i>
            </li>
          </ul>
          <span className="c-grey d-block mt-20 txt-c fs-13 fw-bold">
            This Is Your Current Plan
          </span>
        </div>
      </div>
    </>
  );
}
